...
func addText(params WaterInputParams) {
	for i := 0; i < len(params.Paths); i++ {
		...
		textWidth := advanceTextWidth(fontObj, params.TextWaterInputParams)
...
// 根据设置的字体大小计算文字水印所占的宽度
func advanceTextWidth(fontObj *truetype.Font, params TextWaterInputParams) int {
...
// 设置绘制文字水印的上下文
func drawTextContextSet(fontObj *truetype.Font, textWidth int, dst *image.NRGBA, params WaterInputParams) {
	// 创建一个新的绘图上下文
	...
	*/
	...
	case common.Center:
		pt = freetype.Pt(dst.Bounds().Max.X/2-textWidth/2+params.Offset.Y,
			dst.Bounds().Max.Y/2-int(params.TextWaterInputParams.FontSize)+params.Offset.Y)

	case common.TopLeft:
		pt = freetype.Pt(0+params.Offset.X, 0+int(params.TextWaterInputParams.FontSize)+params.Offset.Y)

	case common.TopRight:
		pt = freetype.Pt(dst.Bounds().Max.X-textWidth+params.Offset.X,
			0+int(params.TextWaterInputParams.FontSize)+params.Offset.Y)

	case common.BottomLeft:
		pt = freetype.Pt(0+params.Offset.X,
			dst.Bounds().Max.Y-int(params.TextWaterInputParams.FontSize)+params.Offset.Y)
	...
	// 绘在位置点制水印文字
	_, err := c.DrawString(params.TextWaterInputParams.Text, pt)
	if err != nil {
		return
	}
}
